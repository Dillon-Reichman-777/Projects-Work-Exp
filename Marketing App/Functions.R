...
ConfusionMatrixInfo_train <- function( data, predict, actual, cutoff )
{	
  # extract the column ;
  ...
  plot <- ggplot( result, aes( actual, predict, color = type ) ) + 
    ...
    ggtitle( sprintf( "Confusion Matrix for the training data set with Cutoff at %.2f", cutoff ) )
  
  return( list( data = result, plot = plot ) )
}

ConfusionMatrixInfo_test <- function( data, predict, actual, cutoff )
{	
  # extract the column ;
  # relevel making 1 appears on the more commonly seen position in 
  # a two by two confusion matrix	
  ...
  plot <- ggplot( result, aes( actual, predict, color = type ) ) + 
    ...
    scale_color_discrete( breaks = c( "TP", "FN", "FP", "TN" ) ) + # ordering of the legend 
    guides( col = guide_legend( nrow = 2 ) ) + # adjust the legend to have two rows  
    ggtitle( sprintf( "Confusion Matrix for the test data set with Cutoff at %.2f", cutoff ) )
...
ROCInfo <- function( data, predict, actual, cost.fp, cost.fn )
{
  ...
  cost <- perf@x.values[[1]] * cost.fp * sum( data[[actual]] == "No") + 
    ( 1 - perf@y.values[[1]] ) * cost.fn * sum( data[[actual]] == "Yes" )
  ...
  auc <- round(performance( pred, "auc" )@y.values[[1]],4)
  ...
  plot <- arrangeGrob( roc_plot, cost_plot, ncol = 2,
                       top = textGrob( sprintf("Optimal cutoff: %.2f, AUC: %.2f", best_cutoff, auc), gp = gpar( fontsize = 16, fontface = "bold" ) ) )
  
  # plot <- arrangeGrob( fig1,fig2, ncol = 2,
  #                      top = textGrob( sprintf( "Cutoff at %.2f, AUC = %e", best_cutoff, auc ), gp = gpar( fontsize = 16, fontface = "bold" ) ) )
  # 
